/**
 * Rabby Wallet Integration for Sonic Labs
 * Optimized for seamless Sonic Labs experience
 */

class RabbyIntegration {
    constructor() {
        this.provider = null;
        this.signer = null;
        this.contracts = {};
    ...
    async init() {
        console.log('🔗 Initializing Rabby Wallet integration for Sonic Labs...');
        ...
        if (typeof window.ethereum !== 'undefined') {
            console.log('✅ Web3 wallet detected:', this.walletType);
    ...
    detectWallet() {
        if (typeof window.ethereum !== 'undefined') {
            ...
            if (window.ethereum.isRabby) {
                ...
                console.log('🎯 Rabby Wallet detected - optimal for Sonic Labs!');
            } else if (window.ethereum.isMetaMask) {
                this.walletType = 'metamask';
                console.log('🦊 MetaMask detected - will work but not optimal');
            } else {
    ...
    async connectWallet() {
        try {
            ...
            if (accounts.length > 0) {
                ...
                if (network.chainId !== 250 && network.chainId !== 4000) {
                    console.log('🔄 Not on Sonic Labs, switching automatically...');
                    await this.switchToSonicLabs();
                } else {
                    // Already on Sonic Labs
                    this.network = network.chainId === 250 ? 'testnet' : 'mainnet';
                    console.log('✅ Already on Sonic Labs', this.network);
                }
    ...
    handleNetworkChange(chainId) {
        ...
        if (chainId === '0xFA' || chainId === '0xFA0') {
            this.network = chainId === '0xFA' ? 'testnet' : 'mainnet';
            this.initializeContracts();
        } else {
    ...
    showWalletPrompt() {
        ...
        prompt.innerHTML = `
            ...
            ">
                ...
                <p>To play BlockZone games and earn rewards, you need a Web3 wallet connected to Sonic Labs Network.</p>
                
                <div style="margin: 1.5rem 0;">
                    <h4>🎯 Recommended: Rabby Wallet</h4>
                    <p style="font-size: 0.9rem; color: #ccc;">Best experience with Sonic Labs</p>
                    <a href="https://rabby.io" target="_blank" style="
                        ...
                        margin: 0.5rem;
                        font-weight: 600;
                    ">🚀 Install Rabby Wallet</a>
                </div>
                
                <div style="margin: 1.5rem 0;">
                    <h4>🦊 Alternative: MetaMask</h4>
                    <p style="font-size: 0.9rem; color: #ccc;">Will work but requires manual setup</p>
    ...
    // Check if using optimal wallet
    isOptimalWallet() {
        return this.walletType === 'rabby';
    }
}

// Create global instance
window.rabbyIntegration = new RabbyIntegration();

export { RabbyIntegration }; 