...
export class GameEngine {
    ...
    startGame(mode = 'practice') {
        ...
        // Start scoring system // User requested to remove this for now
    ...
    async gameOver() { // Add 'async' here
        ...
        }        // Submit score to leaderboard (OFFLINE MODE - DISABLED)
        try {
            // const result = await leaderboard.submitScore(scoreProof); // Offline mode

            // Offline mode - skip leaderboard submission
            // console.log('Offline mode: Skipping leaderboard submission');

            // if (result.success) {
            //     leaderboardUI.showSubmissionResult(result);
            // } else {
            //     // console.error('Leaderboard submission failed:', result.reason || 'Unknown error');
            // }
        } catch (err) {
